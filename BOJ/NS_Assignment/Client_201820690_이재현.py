...
def start_client():
    ...
    server_dhe = int(client.recv(1024).decode())

    # 서버로부터 RSA 암호화된 DHE 파라미터 및 공개키 수신
    encrypted_data = client.recv(256)
    cipher_rsa = PKCS1_OAEP.new(server_public_key)
    decrypted_data = cipher_rsa.decrypt(encrypted_data).decode()
    n_dec, g_dec, server_dhe_dec = map(int, decrypted_data.split(','))

    # DHE 파라미터 및 공개키의 무결성 검증
    if n == n_dec and g == g_dec and server_dhe == server_dhe_dec:
